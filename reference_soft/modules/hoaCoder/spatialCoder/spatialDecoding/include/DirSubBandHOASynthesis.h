...
/*
 ...
 $Id: DirSubBandHOASynthesis.h 196 2015-10-12 11:45:11Z technicolor-ks $
*/
...
class DirSubBandHOASynthesis 
{
    public:
		DirSubBandHOASynthesis();

		bool init(  unsigned int p_uiNoOfSamplesPerQMFBand,
		...
		bool process( const std::vector<std::vector<std::vector<std::complex<FLOAT> > > >  &p_rvvvcFFreqDomTruncHOAInputBuffer,
    ...
    private:
        ...
        bool m_bDoDirSubBandPredInPrevFrame;
        ...
        std::vector<std::map<unsigned int, unsigned int> > m_vmuiLastActiveSubBandDirAndGridDirIncides;
        std::vector<std::vector<std::vector<std::complex<FLOAT> > > >  m_vvvcFLastComplexDirPredictionCoeffsMat;
};