...
/*
 ...
 $Id: ChannelReAssignment.h 157 2015-01-16 14:00:35Z technicolor-kf $
*/
#pragma once
...
class ChannelReAssignment
{
	public:

			ChannelReAssignment();

			bool init(unsigned int p_uiNoOfChannels, 
					  unsigned int p_uiMaxNoOfDirSigs, 
					  unsigned int p_uiNoOfHOACoeffs, 
					  unsigned int p_uiFrameSize);

			bool process(const	std::vector<std::vector<FLOAT> > & p_rvvFInputAllDecodedSigsFrame,
	...
	private:
		...
		unsigned int m_uiNoOfAvailableChannels;
		...
		bool checkBufferSizes(const std::vector<std::vector<FLOAT> > & p_rvvFInputAllDecodedSigsFrame,
							  const std::vector<std::vector<FLOAT> > & p_rvvFOutputDecDirSigsFrame,
							  const std::vector<std::vector<FLOAT> > & p_rvvFOutputDecAmbHOACoeffsFrame);
