...
/*
 ...
 $Id: FinalHOAComposition.h 196 2015-10-12 11:45:11Z technicolor-ks $
*/
#pragma once
...
class FinalHOAComposition
{
	...
	private:
        ...
        unsigned int    m_uiMaxNoOfPARHOACoeffs;

		unsigned int m_uiFrameSize;

		unsigned int m_uiNoOfQMFDelaySamples;

        //std::vector<std::vector<FLOAT> > m_vvFPreDomPlusAmbientCurrentHOAFrame;
        //std::vector<std::vector<FLOAT> > m_vvFPreDomPlusAmbientLastShortHOAFrame;

        std::vector<std::vector<FLOAT> > m_vvFPrelimComposedHOACoeffsLastShortHOAFrame;

		// member functions
		bool checkBufferSizes(	const std::vector<std::vector<FLOAT> > & p_rvvFInputPreDomSoundHOACoeffsFrame, 
                                ...
                                const std::vector<std::vector<FLOAT> > & p_rvvFInputPredDirHOACoeffsFrame,
									  std::vector<std::vector<FLOAT> > & p_rvvFOutputComposedHOACoeffsFrame);
};
