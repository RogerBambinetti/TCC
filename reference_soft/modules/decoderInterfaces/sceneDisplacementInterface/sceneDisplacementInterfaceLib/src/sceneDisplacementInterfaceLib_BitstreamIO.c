...
int SD_DecInt_writeBits(H_SD_BITSTREAM_ENCODER h_bitstream, unsigned int data, int no_bits)
{
  int i = 0; 
...
{
  H_SD_BITSTREAM_ENCODER temp = (H_SD_BITSTREAM_ENCODER)calloc(1, sizeof(struct _sdBitstreamEncoder));
...
{
  H_SD_BITSTREAM_DECODER temp = (H_SD_BITSTREAM_DECODER)calloc(1, sizeof(struct _sdBitstreamDecoder));
...
int SD_DecInt_readBits(H_SD_BITSTREAM_DECODER h_bitstream, unsigned int *data, int no_bits)
{
...
int SD_DecInt_convertIfNegative(unsigned int value, int no_bits)
{
...
int SD_DecInt_readEscapedValue(H_SD_BITSTREAM_DECODER h_bitstream, unsigned int *data, int nbits1, int nbits2, int nbits3)
{
  unsigned int value;
  ...
  no_bitsRead += SD_DecInt_readBits(h_bitstream,&value,nbits1);
  if (value == pow(2.0f,nbits1)-1)
  {
    no_bitsRead += SD_DecInt_readBits(h_bitstream,&valueAdd1,nbits2);
    value += valueAdd1;
    if (valueAdd1 == pow(2.0f,nbits2)-1)
    {
      no_bitsRead += SD_DecInt_readBits(h_bitstream,&valueAdd2,nbits3);
      value += valueAdd2;
    }
  }
...
int SD_DecInt_writeEscapedValue(H_SD_BITSTREAM_ENCODER h_bitstream, unsigned int data, int nbits1, int nbits2, int nbits3)
{
  ...
  if (temp >= pow(2.0f,nbits1)-1)
  {
    ...
    no_bitsWritten += SD_DecInt_writeBits(h_bitstream,value,nbits1);
    rest = temp - value;
    if (rest >= (unsigned int)pow(2.0f,nbits2)-1)
    {
      valueAdd1 = (unsigned int)pow(2.0f,nbits2)-1;
      no_bitsWritten += SD_DecInt_writeBits(h_bitstream,valueAdd1,nbits2);
      rest2 = rest - valueAdd1;
      if (rest >= (unsigned int)pow(2.0f,nbits3)-1)
      {
        valueAdd2 = (unsigned int)pow(2.0f,nbits3)-1;
        no_bitsWritten += SD_DecInt_writeBits(h_bitstream,valueAdd2,nbits3);
      }
      else
      {
        valueAdd2 = rest2;
