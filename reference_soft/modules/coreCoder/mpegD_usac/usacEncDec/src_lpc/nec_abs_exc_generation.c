...
)
{
   ...
   if ( (c_subframe % n_subframes) == 0 && prev_vad != 1 ){
       for ( i = 0; i < pitch_max+pitch_iftap+1; i++ ) {
	   mem_past_exc[i] = 0.0;
       }
   }
   ...
   if(c_subframe==0) {
	 ...
	 nec_dec_rms(qxnorm, n_subframes,
		     (float)NEC_RMS_MAX_U, (float)NEC_MU_LAW_U,
                     rmsbit, (long)rms_index, flag_rms, pqxnorm);
      } else {
	 nec_dec_rms(qxnorm, n_subframes,
	  ...
	  && ((signal_mode == 1) || (signal_mode == 0)) )
	 	errConLoadMode_received( (unsigned long *)&vu_flag );
#endif
#endif
   }
#ifdef VERSION2_EC	/* SC: 1, EC: RMS, MPE */
   /* error concealment of RMS */
   {
	int	act_flag;

	act_flag = getErrAction();
	if( act_flag & EA_RMS_PREVIOUS ){
	    float last_rms;

	    errConLoadRms( &last_rms );
	    if( act_flag & EA_RMS_REDUCE ){
		float dB1 = (float)-3276/8192;
		float dB2 = (float)-9830/8192;

#ifdef	EC_SF_DEPEND
		if( errConGetFsMode() == fs16kHz ){
		    if( sbfrm_size <= EC_SF_DEPEND_SZ1 ){
		        if( sbfrm_size <= EC_SF_DEPEND_SZ2 ){
...
#endif
		if( getErrState()*n_subframes*sbfrm_size <= 320 )
		    last_rms *= pow( 10.0, dB1/20.0 );	/* -0.4dB */
		else
		    last_rms *= pow( 10.0, dB2/20.0 );	/* -1.2dB */
	    }
	    qxnorm[c_subframe] = last_rms;
	}
	errConSaveRms( qxnorm[c_subframe] );
   }
#endif
...
#ifdef VERSION2_EC	/* SC: 1, EC: GAIN, BRS */
      if( getErrAction() & EA_GAIN_PREVIOUS ){
         g_pc = 1.0;
         g_ec = 0.0;
      }
