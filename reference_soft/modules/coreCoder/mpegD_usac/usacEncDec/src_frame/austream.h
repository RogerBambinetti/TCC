...
typedef enum {
  ...
  AU_STREAM_FORMAT_AU = 0, 
  AU_STREAM_FORMAT_WAVE 
...
AuStream *AuOpenWrite (
  ...
  int int24flag,                /* in: write 24 bit output (WAVE only) */
  int bWriteIEEEFloat);         /* in: bWriteIEEEFloat (WAVE only) */
				/* returns: */
				/*  audio stream (handle) */
				/*  or NULL if error */
...
void AuWriteDataFloat (AuStream *stream,	/* in: audio stream (handle) */
                       float *data,		/* in: data[] */
                       long numSample);         /* in: number of samples to be written */
