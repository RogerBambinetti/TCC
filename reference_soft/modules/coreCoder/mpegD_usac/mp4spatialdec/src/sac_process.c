...
#ifndef RM0_3D_BUGFIX_HYBRID_RESIDUAL_BANDLIMIT
#define RM0_3D_BUGFIX_HYBRID_RESIDUAL_BANDLIMIT
#endif
...
void  SpatialDecDecorrelate(spatialDec* self) {
    ...
    float(*wWetRealTemp)[MAX_TIME_SLOTS][MAX_HYBRID_BANDS] = malloc(sizeof(float[MAX_M2_INPUT][MAX_TIME_SLOTS][MAX_HYBRID_BANDS]));
    float(*wWetImagTemp)[MAX_TIME_SLOTS][MAX_HYBRID_BANDS] = malloc(sizeof(float[MAX_M2_INPUT][MAX_TIME_SLOTS][MAX_HYBRID_BANDS]));

    float(*vTsdReal)[MAX_HYBRID_BANDS] = malloc(sizeof(float[MAX_TIME_SLOTS][MAX_HYBRID_BANDS]));
    float(*vTsdImag)[MAX_HYBRID_BANDS] = malloc(sizeof(float[MAX_TIME_SLOTS][MAX_HYBRID_BANDS]));
    float(*wTsdReal)[MAX_HYBRID_BANDS] = malloc(sizeof(float[MAX_TIME_SLOTS][MAX_HYBRID_BANDS]));
    float(*wTsdImag)[MAX_HYBRID_BANDS] = malloc(sizeof(float[MAX_TIME_SLOTS][MAX_HYBRID_BANDS]));
    float tsdMulReal, tsdMulImag;
    int tsdStartBand = 7;

    assert(NULL != wWetRealTemp && NULL != wWetImagTemp && NULL != vTsdReal && NULL != vTsdImag  && NULL != wTsdReal  && NULL != wTsdImag);
...
void SpatialDecMergeResDecor(spatialDec* self) {
  ...
  for (ts=0; ts< self->timeSlots; ts++) {
    for (qs=0; qs< self->hybridBands; qs++) {
...
#ifndef RM0_3D_BUGFIX_HYBRID_RESIDUAL_BANDLIMIT
        /* limit hybrid residual to 6 bands for bitidentity with submission */
        if( indx < ((self->resBands[res]==1)?6:self->resBands[res]) ) {
#else
        /* bugfix: hybrid residual should be fullband, but violates bitidentity due to filterbank noise in the higher frequencies */                   
        if( (indx < self->resBands[res]) || (self->resBands[res]==1) ) {
#endif
      ...
      for(row = self->numDirektSignals; row < self->numWChannels; row++){
        if( (self->resBands[SpatialDecGetResidualIndex(self, row)])!= 1 && (indx < self->resBands[SpatialDecGetResidualIndex(self, row)]) ) {  
          self->wWetReal[row][ts][qs] = 0.0f;
          self->wWetImag[row][ts][qs] = 0.0f;
        }
      }
...
void SpatialDecCopyXtoY(spatialDec* self) {
  int ts,qs, indx;
  ...
  for (ts=0; ts< self->timeSlots; ts++) {
    for (qs=0; qs < self->hybridBands; qs++) {
      ...
      self->hybOutputRealDry[0][ts][qs] = self->hybInputReal[0][ts][qs];
      self->hybOutputImagDry[0][ts][qs] = self->hybInputImag[0][ts][qs];
