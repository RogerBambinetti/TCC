...
#include "iarFormatConverter_main.h"
    
#define IAR_QMFLIB_MAX_CHANNELS 32
#define IAR_QMF_BANDS 64
...
int main(int argc, char *argv[])
{
    ...
    IAR_HANDLE_FORMAT_CONVERTER_PARAMS  hFormatConverter_params     = NULL;
    IAR_HANDLE_FORMAT_CONVERTER_STATE   hFormatConverter_state      = NULL;
    IAR_FORMAT_CONVERTER_MODE           formatConverterMode;
    IAR_FORMAT_CONVERTER_INPUT_FORMAT   formatConverterInputFormat;
    IAR_FORMAT_CONVERTER_OUTPUT_FORMAT  formatConverterOutputFormat;
...
#if IAR
    char*      inFilename3DType           = NULL;            /* a text file name for the rendering3DType bitstream */
    int        rendering3DTypeFlag        = 0;               /* the value of the rendering3DType flag */
    FILE *     fpR3T                      = NULL;            /* file pointer for the rendering3DType bitstream */
    int        elv                        = 35;              /* default elevation of the height channels */
#endif
    ...
    int dmxTypeIdx = 3; /* (passive TD = 0, passive FD = 1, active-PA FD = 3, custom FD (QMF) = 4, active STFT = 5, custom STFT = 6)\n") */
    ...
    fprintf( stdout, "\n"); 
    fprintf( stdout, "***************** MPEG 3D Audio Decoder - Reference Model 3 *******************\n");
    fprintf( stdout, "*                                                                             *\n");
    fprintf( stdout, "*                     Immersive Format Converter Module                       *\n");
    fprintf( stdout, "*                                                                             *\n");
    fprintf( stdout, "*                                  %s                                *\n", __DATE__);
    fprintf( stdout, "*                                                                             *\n");
...
#pragma mark CMDL_PARSING
    ...
    for ( i = 1; i < (unsigned int) argc; ++i )
    {
...
#if IAR
            if    ( outConfIdx <5 || outConfIdx > 6 )
            {
                fprintf (stderr, "Warning: Inappropriate CICP Output Layout for Immersive Format Converter!\n" );
                fprintf (stderr, "         Immersive Format Converter only supports 5.0 and 5.1 layout\n" );
            }
#endif
        ...
        {
          if ( dmxTypeIdx != IAR_FORMAT_CONVERTER_MODE_CUSTOM_FREQ_DOMAIN_PHASE_ALIGN )
          {
        ...
        {
          if ( dmxTypeIdx != IAR_FORMAT_CONVERTER_MODE_CUSTOM_FREQ_DOMAIN_PHASE_ALIGN || dmxTypeIdx != IAR_FORMAT_CONVERTER_MODE_CUSTOM_FREQ_DOMAIN_STFT )
          {
        ...
        /* rendering3DType name */
#if IAR
        else if ( !strcmp(argv[i],"-3DType" ) )        /* Required */
        {
            rendering3DTypeFlag = 1;
            
            if ( argv[i+1] ) 
            {
                inFilename3DType = argv[i+1];
            }
            else 
            {
                fprintf(stderr, "No input filename for rendering3DType flag.\n");
                return -1;
            }                    
            i++;
        }
        else if ( !strcmp(argv[i],"-elv") )        /* Optional */
        {
            elv        = atoi ( argv[i+1] );
            i++;
        }
    ...
    {            
        ...
        fprintf(stderr, "iarformatConverterCmdl Usage:\n\n");
        fprintf(stderr, "Example (time domain interface):      <iarformatConverterCmdl -if in.wav -of out.wav -cicpIn 9 -cicpOut 1 -dmx 0 -d>\n");
        fprintf(stderr, "Example (random setup):               <iarformatConverterCmdl -if in.wav -of out.wav -cicpIn 9 -cicpOut 1 -angleDev dev.txt -dist dist.txt> \n");
        fprintf(stderr, "Example (frequency domain interface): <iarformatConverterCmdl -if in -of out -inConf 3 -outConf 2 -dmx 3 -dmxMtx dmxMtx.txt -info 2> \n");
        fprintf(stderr, "Example (dmx matrix output):          <iarformatConverterCmdl -cicpIn 13 -cicpOut 6 -dmxMtxOutput downmixMtxOut.txt\n");
#if IAR
        fprintf(stderr, "Example (rendering3DType):            <iarformatConverterCmdl -cicpIn 13 -cicpOut 6 -3DType renderingType.txt\n\n");
#endif
        fprintf(stderr, "Example (mix matrix output):          <iarformatConverterCmdl -cicpIn 10 -cicpOut 2 -mixMtxOutput mixMtxOut.txt -dmxMtx dmxMtx.txt\n\n");
        ...
        fprintf(stderr, "-dmx      <dmxType>            dmx type idx (passive TD = 0 (default), passive FD = 1, active-PA FD = 3, custom FD = 4 (needs -aes and -pas switches).\n");
        fprintf(stderr, "                               active STFT = 5, custom STFT = 6 (needs -aes and -pas switches).\n");
...
#if IAR
        fprintf(stderr, "-3DType   <renderingType.txt>  input decoded \"rendering3DType\" bitstream decoded from the core coder. \n");
        fprintf(stderr, "                                 format: [frame x 1], 1 for the spatial elevation rendering and 0 for the timbral elevation rendering.\n");
        fprintf(stderr, "-elv       <21,60>              target elevation of the height channels: Value between 21 and 60 in degree\n");
#endif
        return -1;
    }
...
#pragma mark OPEN_FORMAT_CONVERTER
    ...
    switch (dmxTypeIdx) {
        case 0:
            formatConverterMode = IAR_FORMAT_CONVERTER_MODE_PASSIVE_TIME_DOMAIN;
            /* qmf IO */
            if (qmfIOFlag) {
            ...
            break;
        case 1:
            formatConverterMode = IAR_FORMAT_CONVERTER_MODE_PASSIVE_FREQ_DOMAIN;
            break;
        case 3:
            formatConverterMode = IAR_FORMAT_CONVERTER_MODE_ACTIVE_FREQ_DOMAIN_PHASE_ALIGN;
            break;
        case 4:
            formatConverterMode = IAR_FORMAT_CONVERTER_MODE_CUSTOM_FREQ_DOMAIN_PHASE_ALIGN;
            if ( phaseAlignStrength == -1 || adaptiveEQStrength == -1 )
            {
              fprintf(stderr, "Error: Please set both switches (-pas and -eas) when using custom downmix mode.\n");
              fprintf(stderr, "Default values will be used now: (pas = 0, eas = 0)\n");
              return -1;
            }
            break;
        case 5:
            formatConverterMode = IAR_FORMAT_CONVERTER_MODE_ACTIVE_FREQ_DOMAIN_STFT;
            break;
        case 6:
            formatConverterMode = IAR_FORMAT_CONVERTER_MODE_CUSTOM_FREQ_DOMAIN_STFT;
    ...
    switch (inConfIdx) {
        case 0:
            formatConverterInputFormat = IAR_FORMAT_CONVERTER_INPUT_FORMAT_GENERIC;
            break;
        default:
            formatConverterInputFormat = IAR_FORMAT_CONVERTER_INPUT_FORMAT_LISTOFCHANNELS;
            break;
    }
    ...
    switch (outConfIdx) {
        case 0:
            formatConverterOutputFormat = IAR_FORMAT_CONVERTER_OUTPUT_FORMAT_GENERIC;
            break;
        default:
            formatConverterOutputFormat = IAR_FORMAT_CONVERTER_OUTPUT_FORMAT_LISTOFCHANNELS;
            break;
    }

    /* sanity check for generic input/output formats */
    if( formatConverterInputFormat == IAR_FORMAT_CONVERTER_INPUT_FORMAT_GENERIC || formatConverterOutputFormat == IAR_FORMAT_CONVERTER_OUTPUT_FORMAT_GENERIC ) {
    ...
    switch(formatConverterMode)
    {
        ...
        switch (hybMode) {
            case 0:
                ...
                formatConverterCenterFrequencies = iarF_bands_nrm_71;
                break;
            case 2:
                formatConverterNumFreqBands = 77;
                formatConverterCenterFrequencies = iarF_bands_nrm_77;
                break;
            case -1:
                formatConverterNumFreqBands = 64;
                formatConverterCenterFrequencies = iarF_bands_nrm_64;
                break;
            case 1:
            default:
        ...
        case IAR_FORMAT_CONVERTER_MODE_CUSTOM_FREQ_DOMAIN_STFT:
        {
            ...
            stftFormatConverterErbFreqIdx       = iar_erb_freq_idx_256_58;
            formatConverterCenterFrequencies    = iarF_bands_nrm_stft_erb_58;
            stftWindow                          = iar_stftWindow_512;

            break;
        }
        case IAR_FORMAT_CONVERTER_MODE_PASSIVE_TIME_DOMAIN:
        {
            formatConverterFrameSize = 1024;   
            break;
        }
...
/*******************************************************************/
    if((outConfIdx==-1) || (outConfIdx>0)){
        ...
        iar_api_chid_t *channel_vec = NULL;
        ...
        errorFlag = iar_formatConverterMatchChConfig2Channels(outputChConfig,1, \
        ...
        if (formatConverterOutputFormat != IAR_FORMAT_CONVERTER_OUTPUT_FORMAT_GENERIC) {
            iar_formatConverterSetInOutFormat(1, Nout_int, channel_vec);
        }
...
/*******************************************************************/
    if((inConfIdx==-1) || (inConfIdx>0)){
        ...
        if(formatConverterOutputFormat != IAR_FORMAT_CONVERTER_OUTPUT_FORMAT_GENERIC) {
            /* match known channels in channel config */
            errorFlag = iar_formatConverterMatchChConfig2Channels(inputChConfig,0, \
                numChannels,numLfes,1,&channel_vec,&numUnknownCh,&unknownCh_vec, \
            ...
            if ((numUnknownCh > 0) && (formatConverterOutputFormat != IAR_FORMAT_CONVERTER_OUTPUT_FORMAT_GENERIC)){
            ...
            iar_formatConverterSetInOutFormat(0, Nin_int, channel_vec);   
...
/*******************************************************************/
    ...
    /* open format converter */
    if ( iar_formatConverterOpen(   formatConverterMode, 
...
#if IAR
    /*******************************************************************/
    /*    read rendering3DType (necessary bitstream)                   */
    /*       -> assumes always TRUE if no bitstream is provided          */
    /*******************************************************************/
    if ( rendering3DTypeFlag == 1 )
    {
        fpR3T    = fopen ( inFilename3DType, "rt" );
        if    ( !fpR3T )
        {
            fprintf(stderr, "Unable to find the rendering3DType data\n");
            return -1;
        }
    }
#endif
    ...
    if (angleDeviationFlag == 1) {
        ...
        errorFlag = iarReadAngleDeviationsFromFile( inFilenameAngleDeviation, azimuthDeviationExt, elevationDeviationExt, formatConverterNumOutputChans );
        if ( 0 != errorFlag )
        {
            fprintf(stderr, "Error during iarReadAngleDeviationsFromFile.\n");
            return -1;
        } 
    ...
    if (distanceFlag == 1) {
        ...
        errorFlag = iarReadDistancesFromFile( inFilenameDistance, distance, formatConverterNumOutputChans );
        if ( 0 != errorFlag )
        {
            fprintf(stderr, "Error during iarReadDistancesFromFile.\n");
            return -1;
        }        
    ...
    errorFlag = iar_formatConverterInit(
    ...
    if ( errorFlag != 0 )
    {
        if(errorFlag==-2){
            ...
            if ( iar_formatConverterClose ( &hFormatConverter_params, &hFormatConverter_state ) != 0)   
            {
                fprintf(stderr,"Error in iar_formatConverterClose. Exiting application. \n");
                return 1;
            }
            if ( iar_formatConverterOpen(   formatConverterMode, 
                                formatConverterInputFormat,
    ...
    if(dmxTypeIdx == 4 || dmxTypeIdx == 6)
    {
        ...
        {
        case IAR_FORMAT_CONVERTER_MODE_CUSTOM_FREQ_DOMAIN_PHASE_ALIGN:
            iar_setCustomDownmixParameters( hFormatConverter_state, adaptiveEQStrength, phaseAlignStrength);
            break;
        case IAR_FORMAT_CONVERTER_MODE_CUSTOM_FREQ_DOMAIN_STFT:
            iar_setCustomDownmixParameter_STFT( hFormatConverter_state, adaptiveEQStrength);
            break;
        default:;
        }
    }
    ...
    if (distanceFlag == 1) {
        ...
        /* get trim params */
        if (iar_formatConverterGetTrimParams(trimGains, trimDelays, hFormatConverter_params) != 0)
        {
    ...
    if (outChannelVbapFlag) {                  
        ...
        iar_directMapLFEs(dmxMtx, Nin_int, inputChConfig, Nout_int, outputChConfig);

        /* post-process gains to reduce phantom sources */        
        iar_formatConverterPostprocessDmxMtx(dmxMtx,formatConverterNumInputChans,formatConverterNumOutputChans);

        fprintf(stderr, "\nApplying gVBAP DMX matrix ... changing MIX matrix ... \n");       
        errorFlag = iar_formatConverterSetDmxMtx((const float **)dmxMtx, hFormatConverter_params, hFormatConverter_state);   
        if ( 0 != errorFlag )
        {
            fprintf(stderr, "Error during iar_formatConverterSetDmxMtx.\n");
            return -1;
        }
        errorFlag = iar_formatConverterSetDmxMtx_STFT(hFormatConverter_params,  hFormatConverter_state);
        if ( 0 != errorFlag )
        {
            fprintf(stderr, "Error during iar_formatConverterSetDmxMtx_STFT.\n");
            return -1;
        }
    }
    ...
    if (inChannelVbapFlag && (!outChannelVbapFlag)) {
        ...
        /* direct mapping of LFE(s) to LFE(s) if LFE(s) are present in target setup */
        iar_directMapLFEs(tmpDmxMtx, numUnknownCh, unknownChannelsGeo, Nout_int, outputChConfig);
        ...
        errorFlag = iar_formatConverterAddDmxMtx((const float **)dmxMtx, hFormatConverter_params, hFormatConverter_state);   
        if ( 0 != errorFlag )
        {
            fprintf(stderr, "Error during iar_formatConverterAddDmxMtx.\n");
            return -1;
        }
    }
    ...
    if (dmxMtxInputFlag) {
        ...
        errorFlag = iarReadDmxMtxFromFile( inFilenameDmxMtx, dmxMtx, formatConverterNumInputChans, formatConverterNumOutputChans );
        if ( 0 != errorFlag )
        {
            fprintf(stderr, "Error during iarReadDmxMtxFromFile.\n");
            return -1;
        }
    ...
    if (extEqFlag == 1) {           
        ...
        int numEQs = 0;
        iar_eqParamsStruct *eqParams  = NULL;
        int *eqMap = NULL;

        errorFlag = iarReadEqParamsFromFile( inFilenameEqParams, formatConverterNumInputChans, &numEQs, &eqParams, &eqMap );
        if ( 0 != errorFlag )
        {
            fprintf(stderr, "Error during iarReadEqParamsFromFile.\n");
            return -1;
        }        

        fprintf(stderr, "\nApplying external EQ parameters...\n");     
        errorFlag = iar_formatConverterSetEQs( numEQs, eqParams, eqMap, formatConverterCenterFrequencies, (float) samplingRate, hFormatConverter_params, hFormatConverter_state);
        if ( 0 != errorFlag )
        {
            fprintf(stderr, "Error during iar_formatConverterSetEQs.\n");
            return -1;
        }
...
#pragma mark DELAY
    ...
    /* module delay */      
    if(formatConverterMode != IAR_FORMAT_CONVERTER_MODE_ACTIVE_FREQ_DOMAIN_STFT
    && formatConverterMode != IAR_FORMAT_CONVERTER_MODE_CUSTOM_FREQ_DOMAIN_STFT)
    {
        if (ldMode) {
            switch (hybMode) {
    ...
    if (formatConverterMode != IAR_FORMAT_CONVERTER_MODE_PASSIVE_TIME_DOMAIN ) 
    {
    ...
    if (plotInfo > 0) {
        ...
        /* display parameters */
        iar_formatConverterDisplayParams( plotInfo-1, hFormatConverter_params );   

        fprintf(stderr,"Format Converter Lib  Info: %s \n",iar_formatConverterGetInfo());  
        fprintf(stderr,"Format Converter Cmdl Info: %s \n",iarFormatConverterCmdlGetInfo());      
...
#pragma mark DMX_MIX_MATRIX 
    ...
    if (dmxMtxOutputFlag) {
        ...
        /* get mix matrix */
        iar_formatConverterGetDmxMtx(dmxMtx, hFormatConverter_params);
    ...
    if (mixMtxOutputFlag) {
        ...
        /* get mix matrix */
        iar_formatConverterGetMixMtx(mixMtx, hFormatConverter_params);
...
#pragma mark INIT_QMF
    ...
    if (formatConverterMode == IAR_FORMAT_CONVERTER_MODE_ACTIVE_FREQ_DOMAIN_PHASE_ALIGN 
        || formatConverterMode == IAR_FORMAT_CONVERTER_MODE_CUSTOM_FREQ_DOMAIN_PHASE_ALIGN
        || formatConverterMode == IAR_FORMAT_CONVERTER_MODE_PASSIVE_FREQ_DOMAIN) 
    {
        ...
        hybFilterState = (QMFLIB_HYBRID_FILTER_STATE *) calloc( IAR_QMFLIB_MAX_CHANNELS, sizeof ( QMFLIB_HYBRID_FILTER_STATE ) );	
        ...
        for (i=0; i<IAR_QMFLIB_MAX_CHANNELS; i++) {    
   ...
   
   {
    ...
    do  /*loop over all frames*/
    {
        ...
        if (formatConverterMode != IAR_FORMAT_CONVERTER_MODE_PASSIVE_TIME_DOMAIN ) 
        {
            ...
            {
                case IAR_FORMAT_CONVERTER_MODE_PASSIVE_TIME_DOMAIN:
                case IAR_FORMAT_CONVERTER_MODE_INVALID:
                ...
                /* hybrid QMF */
                if (hybMode != -1) { 
                    if (iar_formatConverterProcess_freqDomain((const float**)inputBufferFreq_real, 
                                                              ...
                                                              fpR3T) != 0 )
                    {
                        fprintf(stderr,"Error in iar_formatConverterProcess_freqDomain. Exiting application. \n");
                        return 1;
                    }
                    /* QMF */
                } else {
                    if (iar_formatConverterProcess_freqDomain((const float**)inputBufferFreqQMF_real, 
                ...
                {
                    unsigned int s;
                    int rendering3DType = 0;
                    unsigned int realIdx, imagIdx;

#if IAR
                    if (FormatConverterFrame(fpR3T, &rendering3DType))
                    {
                        fprintf(stderr,"Error in FormatConverterFrame. Exiting application. \n");
                        return 1;
                    }
#endif
                    ...
                    for(j=0;j<stftFormatConverterNumTimeSlots;j++)
                    {
                        ...
                        iar_formatConverterProcess_freqDomain_STFT((const float **)inputBufferStft,
                                                                    outputBufferStft,
                                                                    hFormatConverter_params,
                                                                    hFormatConverter_state,
                                                                    rendering3DType);
            ...
            if (iar_formatConverterProcess_timeDomain( (const float**)inBuffer, 
                                                  outBuffer, 
                                                  hFormatConverter_params, 
                                                  hFormatConverter_state) != 0 )   
            {
                fprintf(stderr,"Error in iar_formatConverterProcess_timeDomain. Exiting application. \n");
                return 1;
            } 
    ...
    ratio = (float) (elapsed_time + elapsed_time_QMF+elapsed_time_STFT) / inputDuration;
    
    if (plotInfo > 0) {        
        
        fprintf(stderr,"\nProcessed %d frames ( %.2f s ) in %.2f s.\n", frameNo, inputDuration, (float) (elapsed_time+elapsed_time_QMF+elapsed_time_STFT) );
        fprintf(stderr,"Processing duration / Input duration (iarFormatConverterCmdl) = %4.2f %% .\n", (ratio * 100) ); 
...
{   
    ...
    *eqParams = (iar_eqParamsStruct*)calloc(*numEQs,sizeof(iar_eqParamsStruct));        
    ...
    for(n=0; n<*numEQs; n++){
        ...
        (*eqParams)[n].pkFilterParams = (iar_pkFilterParamsStruct*)calloc( (*eqParams)[n].nPkFilter, sizeof( iar_pkFilterParamsStruct ) ); 
...
int FormatConverterFrame(FILE * fpR3T, int* rendering3DType)
{
    ...
    if ( fpR3T && file_ok )
    {
        if ( fgets ( tmp, 16, fpR3T ) )
        {
            /* parse FormatConverterFrame() */
            *rendering3DType = (atoi(tmp) >> 7) & 1;
        }
        else
        {
            fprintf(stderr, "\n================ Error reading fpR3T file! fgets() failed! ==============\n\n");
            file_ok = 0;  /* Just mark that we won't further read the file. */
            error = 1;
        }
    }
