...
#include <stdio.h>
#include <stdlib.h>
#include "iarFormatConverter_phaseAlign.h"
#include <math.h>
...
void iar_phaseAlignProcessFrame(void *handle);
                    ...
                    float eqLimitMin)
{
    ...
    *handle = malloc(sizeof(iar_phaseAligner));
    ...
    for (chA=0;chA<h->numInChans;chA++)
    {
        ...
        h->inWindow[chA] = (iar_compexVector*)malloc(sizeof(iar_compexVector)*h->numBands);
        h->inFrame[chA] = (iar_compexVector*)malloc(sizeof(iar_compexVector)*h->numBands);
        h->nextFirstHalfWindow[chA] = (iar_compexVector*)malloc(sizeof(iar_compexVector)*h->numBands);
        for (band=0;band<h->numBands;band++)
        {
            h->inFrame[chA][band].re = (float*)malloc(sizeof(float)*IAR_HOPSIZE);
            h->inFrame[chA][band].im = (float*)malloc(sizeof(float)*IAR_HOPSIZE);
            h->inWindow[chA][band].re = (float*)malloc(sizeof(float)*2*IAR_HOPSIZE);
            h->inWindow[chA][band].im = (float*)malloc(sizeof(float)*2*IAR_HOPSIZE);
            h->nextFirstHalfWindow[chA][band].re = (float*)calloc(sizeof(float),IAR_HOPSIZE);
            h->nextFirstHalfWindow[chA][band].im = (float*)calloc(sizeof(float),IAR_HOPSIZE);
        }
    }
    ...
    for (chA=0;chA<h->numOutChans;chA++)
    {
        ...
        h->nextOutFrame[chA] = (iar_compexVector*)malloc(sizeof(iar_compexVector)*h->numBands);
        h->outWindow[chA] = (iar_compexVector*)malloc(sizeof(iar_compexVector)*h->numBands);
        for (band=0;band<h->numBands;band++)
        {
            h->outFrame[chA][band].re = (float*)calloc(sizeof(float),IAR_HOPSIZE);
            h->outFrame[chA][band].im = (float*)calloc(sizeof(float),IAR_HOPSIZE);
            h->nextOutFrame[chA][band].re = (float*)calloc(sizeof(float),IAR_HOPSIZE);
            h->nextOutFrame[chA][band].im = (float*)calloc(sizeof(float),IAR_HOPSIZE);
            h->outWindow[chA][band].re = (float*)malloc(sizeof(float)*2*IAR_HOPSIZE);
            h->outWindow[chA][band].im = (float*)malloc(sizeof(float)*2*IAR_HOPSIZE);
        }
    }
    ...
    iar_phaseAlignSetDmxMtx(h, dmxMtx);
        ...
        *energyPr = h->lastRoundLastEnergies[band];
        ...
        for (sample=0;sample<IAR_HOPSIZE;sample++)
        {
            ...
            for (ch=0;ch<h->numInChans;ch++)
            {
                ...
                h->inWindow[ch][band].re[sample+IAR_HOPSIZE] = h->inFrame[ch][band].re[sample] - h->nextFirstHalfWindow[ch][band].re[sample];
                h->inWindow[ch][band].im[sample+IAR_HOPSIZE] = h->inFrame[ch][band].im[sample] - h->nextFirstHalfWindow[ch][band].im[sample];
        ...
        for (chB=0;chB<=chA;chB++)
        {
            if (h->CxRelevant[chA][chB]) /* Process only relevant channel pairs */
            {
                ...
                if (chA == chB)
                {
                    ...
                    {
                        *pr=0.0f;
                        for (sample=0;sample<2*IAR_HOPSIZE;sample++)
                        {
    ...
    for (chA=0;chA<h->numOutChans;chA++)
    {
        for (band=0;band<h->numBands;band++)
        {
            for (sample=0;sample<IAR_HOPSIZE;sample++)
            {
                ...
                h->nextOutFrame[chA][band].re[sample] = h->outWindow[chA][band].re[sample+IAR_HOPSIZE];
                h->nextOutFrame[chA][band].im[sample] = h->outWindow[chA][band].im[sample+IAR_HOPSIZE];
            }
        }
    }
