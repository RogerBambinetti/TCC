...
#include <stdio.h>
#include <stdlib.h>
#include "iarFormatConverter_phaseAlign_lowcplx.h"
#include <math.h>
...
void iar_phaseAlignProcessFrame(void *handle, int r3t);

int iar_phaseAlignInit( void **handle, 
                    ...
                    float eqLimitMin)
{
    ...
    *handle = malloc(sizeof(iar_phaseAligner));
    h = (iar_phaseAligner*)(*handle);
    ...
    iar_phaseAlignSetPAS(3, h);
    iar_phaseAlignSetAES(7, h);
    /* Memory allocations */
    ...
    for (i=0;i<IAR_NUMTEMPVECTORS;i++)
    {
    ...
    for (chA=0;chA<h->numInChans;chA++)
    {
        ...
        h->inFrame[chA] = (iar_compexVector*)malloc(sizeof(iar_compexVector)*h->numBands);
        h->prevInFrame[chA] = (iar_compexVector*)malloc(sizeof(iar_compexVector)*h->numBands);
#if IAR
        h->prevInFrameD2[chA] = (iar_compexVector*)malloc(sizeof(iar_compexVector)*h->numBands);
#endif
        for (band=0;band<h->numBands;band++)
        {
            h->inFrame[chA][band].re = (float*)malloc(sizeof(float)*IAR_HOPSIZE);
            h->inFrame[chA][band].im = (float*)malloc(sizeof(float)*IAR_HOPSIZE);
            h->prevInFrame[chA][band].re = (float*)calloc(sizeof(float),IAR_HOPSIZE);
            h->prevInFrame[chA][band].im = (float*)calloc(sizeof(float),IAR_HOPSIZE);
#if IAR
            h->prevInFrameD2[chA][band].re = (float*)calloc(sizeof(float),IAR_HOPSIZE+D2);
            h->prevInFrameD2[chA][band].im = (float*)calloc(sizeof(float),IAR_HOPSIZE+D2);
#endif
    ...
    {
        h->outFrame[chA] = (iar_compexVector*)malloc(sizeof(iar_compexVector)*h->numBands);
        for (band=0;band<h->numBands;band++)
        {
            h->outFrame[chA][band].re = (float*)calloc(sizeof(float),IAR_HOPSIZE);
            h->outFrame[chA][band].im = (float*)calloc(sizeof(float),IAR_HOPSIZE);
        }
    }
    ...
    iar_phaseAlignSetDmxMtx(h, dmxMtx, dmxMtx2);
...
#if IAR
                    h->dmxMtx2[band][chA][chB]=dmxMtx2[band][chB][chA];
#endif
                    h->dmxMtxNonZero[chA][chB]=1;
                }
            }
...
{
    iar_phaseAlignProcess3D(handle,
                            in_QMF_real,
                            in_QMF_imag,
                            out_QMF_real,
                            out_QMF_imag,
...
#endif
{
    ...
    iar_phaseAligner* h = (iar_phaseAligner*)handle;
    int ch,band;
    ...
    h->collectCounter++;
    if (h->collectCounter == IAR_HOPSIZE)
    {
#if IAR
        char    tmp[16]     = {0};
        ...
        if ( fpR3T )
        {
            if ( fgets ( tmp, 16, fpR3T ) )
            {
                ...
                {
                    iar_phaseAlignProcessFrame(handle,1);
                    h->collectCounter    = 0;
                }
                else 
                {
                    iar_phaseAlignProcessFrame(handle,2);
                    h->collectCounter    = 0;
                }
            }
            else
        ...
        {
            iar_phaseAlignProcessFrame(handle,R3T);
            h->collectCounter    = 0;
        }
#else
        iar_phaseAlignProcessFrame(handle);
        h->collectCounter=0;
#endif
    }
...
void iar_phaseAlignProcessFrame(void *handle, int r3t )
{
    ...
    ****************/
            ...
            if (h->CxRelevant[chA][chB]) /* Process only relevant channel pairs */
            {
                ...
                if (chA == chB)
                {
                    for (band=0;band<h->numBands;band++)
                    {
                        ...
                        im1=0.0f;
                        im2=0.0f;
                        for (sample=0;sample<IAR_HOPSIZE/2;sample++)
                        {
                        ...
                        if (ch >= chB) /* This switch is implemented since only half of the conjugate symmetric matric P is available */
                        {
                            ...
                            for (band=0;band<h->numBands;band++)
                            {
                                ...
                                *pIm +=  dmxMtxEntry * (*pPim);
                                ...
                                {
                                    *pRe +=  h->dmxMtx2[band][chA][ch] * (*pPre);
                                    *pIm +=  h->dmxMtx2[band][chA][ch] * (*pPim);                      
                                }
#else    /* IAR */
                                *pRe +=  h->dmxMtx[band][chA][ch] * (*pPre);
                                *pIm +=  h->dmxMtx[band][chA][ch] * (*pPim);                      
...
#if IAR
                        if    ( r3t == 1 )    EQfactor = h->dmxMtx[band][chA][chB]/(h->epsilon + (float)sqrt(energy));              
                        else                  EQfactor = h->dmxMtx2[band][chA][chB]/(h->epsilon + (float)sqrt(energy));              
#else    /* IAR */
...
#if    IAR
                    if    ( r3t == 1 )    tempFloat = (float) pow(h->dmxMtx[band][chA][chB],2.0f);
                    else                  tempFloat = (float) pow(h->dmxMtx2[band][chA][chB],2.0f);
#else
    ...
    for (chA=0;chA<h->numOutChans;chA++)
    {
        ...
        for (band=0;band<h->numBands;band++)
        {
            for (sample=0;sample<IAR_HOPSIZE;sample++)
            {
        ...
        for (chB=0;chB<h->numInChans;chB++)
        {
            if (h->dmxMtxNonZero[chA][chB])
            {
                for (band=0;band<h->numBands;band++)
                {
                    ...
                    incrementRe = (h->M[chA][chB].re[band] - h->M_current[chA][chB].re[band])/(float)IAR_HOPSIZE;
                    incrementIm = (h->M[chA][chB].im[band] - h->M_current[chA][chB].im[band])/(float)IAR_HOPSIZE;
                    for (sample=0;sample<IAR_HOPSIZE;sample++)
                    {
                        re += incrementRe;
                        im += incrementIm;
#if IAR
                        if  ( ( chB == topIn[TFL] || chB == topIn[TFC] || chB == topIn[TFR] || chB == topIn[TFLA] || chB == topIn[TFRA] ) && midOut[chA] > SW && r3t == 1 )
                        {
                            h->outFrame[chA][band].re[sample] += re*h->prevInFrameD2[chB][band].re[sample];
                            h->outFrame[chA][band].re[sample] -= im*h->prevInFrameD2[chB][band].im[sample];
                            h->outFrame[chA][band].im[sample] += re*h->prevInFrameD2[chB][band].im[sample];
                            h->outFrame[chA][band].im[sample] += im*h->prevInFrameD2[chB][band].re[sample];
                        }
                        else
                        {
    ...
    for (chA=0; chA<h->numInChans; chA++)
    {
        if ( 1 )
        {
            for ( band=0; band<h->numBands; band ++ )
            {
                for ( sample=0; sample<D2; sample++ )
                {
                    h->prevInFrameD2[chA][band].re[sample]        = h->prevInFrameD2[chA][band].re[sample+IAR_HOPSIZE];
                    h->prevInFrameD2[chA][band].im[sample]        = h->prevInFrameD2[chA][band].im[sample+IAR_HOPSIZE];
                }
                for ( sample=0; sample<IAR_HOPSIZE; sample++ )
                {
                    h->prevInFrameD2[chA][band].re[sample+D2]    = h->inFrame[chA][band].re[sample];
                    h->prevInFrameD2[chA][band].im[sample+D2]    = h->inFrame[chA][band].im[sample];
                }
            }
        }
    }
...
#if IAR
            free(h->prevInFrameD2[chA][band].re);
            free(h->prevInFrameD2[chA][band].im);
#endif            
        }
        free(h->inFrame[chA]);
        free(h->prevInFrame[chA]);
#if IAR
        free(h->prevInFrameD2[chA]);
#endif
    }
...
int iar_phaseAlignSetPAS(int PAS, void *handle)
{
  ...
  switch(PAS)
  {
    ...
    case 6:
    case 7:
      fprintf(stderr, "Warning: PAS-Mode not implemented! Downmix will be passive.\n");
    case 0:
      h->PASMax        = 0.0f;
      ...
      break;
    default:
      fprintf(stderr, "Warning: Invalid PAS-Mode. Phase align strenght is set to 0.\n");
      h->PASMax        = 0.0f;
      h->PasCurveSlope = 0.0f;
      h->PasCurveShift = 0.0f;
  }
